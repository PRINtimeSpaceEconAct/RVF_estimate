estimationVectorField_best_h_best_alpha <- function(numGrid, grid_y_Rel, grid_Wy_Rel, 
                                                    y_Rel, Wy_Rel, W, numLag, 
                                                    numGrid.h, numGrid.alpha,
                                                    nrepsimPar=100, min.size.drawnPar=1, 
    ...
    #numGrid.alpha: number of grid for searching the optimal alpha par in adaptive
    ...
    #RSS_for_best_h_alpha: residual sum of squares of all model estimated
    #minRSS: min RSS
    #hOpt: optimal bandwidth associated with the minimum RSS
    #alphaOpt: optimal alpha associated with the minimum RSS
    ...
    #Grid for alpha in [0,0.5]
    alphaStart <- 0.0005
    alphaEnd <- 0.5
    alphaGrid <- log(alphaStart) + (log(alphaEnd) - log(alphaStart)) * (0:(numGrid.alpha-2))/(numGrid.alpha-2)
    alphaGrid <- c(0, exp(alphaGrid))
    
    RSS_for_best_h_alpha <- matrix(0, nrow=numGrid.h, ncol=numGrid.alpha)
    rownames(RSS_for_best_h_alpha) <- hgrid
    colnames(RSS_for_best_h_alpha) <- alphaGrid
    RSS_for_best_h_alpha = matrix_to_stack(RSS_for_best_h_alpha, "RSS", "h", "alpha")
    RSS_for_best_h_alpha <- apply(RSS_for_best_h_alpha, 2, FUN=function (x) {as.numeric(as.character(x))} )
    ...
    RSS_for_best_h_alpha[,3] <- foreach(i=1:(numGrid.h*numGrid.alpha), .combine= cbind, .verbose=FALSE) %dopar%{
        
        print(paste("testing alpha = ",RSS_for_best_h_alpha[i,1], ", h = ",RSS_for_best_h_alpha[i,2],sep=""))
        
        alpha = RSS_for_best_h_alpha[i,1]
        h = RSS_for_best_h_alpha[i,2]
        ...
        # RSS_for_best_h_alpha[i] <- simulation_RSS$RSS 		
        return(simulation_RSS$RSS)
    }
    
    minRSS <- min(RSS_for_best_h_alpha[,3]) 
    positionMin <- which.min(RSS_for_best_h_alpha[,3])
    alphaOpt <- RSS_for_best_h_alpha[positionMin,1]
    hOpt <- RSS_for_best_h_alpha[positionMin,2]
    
    print(paste("finished choice of h and alpha:"))
    print(paste("best h = ",hOpt,", best alpha = ",alphaOpt,sep = ""))
    
    ################################################
    # Estimation for the hOpt and alphaOpt selected via min RSS #
    ################################################
    ...
    list(estimationRVF= estimationRVF, RSS_for_best_h_alpha= RSS_for_best_h_alpha, minRSS= minRSS, hOpt=hOpt, xObs=x, estimatedLastYearDistr= estimatedLastYearDistr, alphaOpt=alphaOpt)
}